...
function edit(id,type){
    if(null!=id){
        ...
        $.get("/auth/getPerm",{"id":id},function(data) {
            // console.log(data);
            if(null!=data){
                $("input[name='permitName']").val(data.permitName);
                $("input[name='permitCode']").val(data.permitCode);
                $("input[name='url']").val(data.url);
                $("input[name='zindex']").val(data.zindex);
                $("textarea[name='description']").text(data.description);
                $("#pid").val(data.pid);
                data.permitType==0?$("input[name='permitType']").val(0).checked:$("input[name='permitType']").val(1).checked;
                layer.open({
                    type:1,
                    title: "更新权限",
                    fixed:false,
