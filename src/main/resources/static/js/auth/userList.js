    ...
    layui.use('table', function(){
        ...
        tableIns=table.render({
            ...
            ,cols: [[
                ...
                ,{field:'isUse', title:'是否禁用',width:95,align:'center',templet:'#useTpl'}
        ...
        //监听在职操作
        form.on('switch(isUseTpl)', function(obj){
            //console.log(this.value + ' ' + this.name + '：'+ obj.elem.checked, obj.othis);
            setUserUse(obj,this.value,this.name,obj.elem.checked);
        });
        //监听工具条
...
//设置用户是否禁用
function setUserUse(obj,id,nameVersion,checked){
    var name=nameVersion.substring(0,nameVersion.indexOf("_"));
    var version=nameVersion.substring(nameVersion.indexOf("_")+1);
    var isUse=checked ? 0 : 1;
    var userIsUse=checked ? "可用":"禁用";
    //是否离职
    layer.confirm('您确定要把用户：'+name+'设置为'+userIsUse+'状态吗？', {
        btn: ['确认','返回'] //按钮
    }, function(){
        $.post("/user/setUserUse",{"id":id,"isUse":isUse,"version":version},function(data){
            if(isLogin(data)){
                if(data=="ok"){
                    //回调弹框
...
function getUserAndRoles(obj,id) {
    //如果已经离职，提醒不可编辑和删除
    if(obj.isUse){
        layer.alert("该用户已经禁用，不可进行编辑；</br>  如需编辑，请设置为<font style='font-weight:bold;' color='green'>可用</font>状态。");
    }else if(obj.del){
